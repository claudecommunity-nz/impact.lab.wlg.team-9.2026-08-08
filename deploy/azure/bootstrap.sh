...
# The connection string requirement is checked further down, at the point where
# it would otherwise prompt — by then we know whether Key Vault already has it,
# which is the normal case on a re-run and needs no input at all.
...
if [[ -z "${ACR_NAME:-}" ]]; then
  ...
  if [[ -n "$EXISTING_ACR" && "$EXISTING_ACR" != "None" ]]; then
    ...
    ACR_NAME="team9signals$(head -c 4 /dev/urandom | od -An -tx1 | tr -d ' \n')"
  fi
fi

# Every other name is derived from the registry's suffix, and each is filled in
# only if missing. That matters on a re-run: an .azure-env written before a
# given name existed still gets topped up, instead of the name silently staying
# unset because the block that would have created it was skipped. Adding a name
# here later works the same way, which is the point.
SUFFIX="${SUFFIX:-${ACR_NAME#team9signals}}"
DNS_LABEL="${DNS_LABEL:-team9-signals-${SUFFIX}}"
# Vault names are globally unique and max 24 characters. The random suffix also
# sidesteps Key Vault soft-delete: a deleted vault holds its name for 90 days,
# so a re-bootstrap with a fixed name would collide with its own ghost.
KEY_VAULT_NAME="${KEY_VAULT_NAME:-team9-kv-${SUFFIX}}"
...
STORAGE_ACCOUNT="${STORAGE_ACCOUNT:-team9st${SUFFIX}}"
...
#
# Written atomically, via a temp file. `cat > file <<EOF` truncates the target
# before it expands the heredoc, so an unset variable in here does not just
# fail — it destroys the names that were already saved.
save_env() {
...
if [[ -z "${MONGO_URI:-}" && "$NONINTERACTIVE" == "1" ]]; then
  fail "no connection string available — set MONGO_URI, or run interactively to be prompted (Key Vault '${KEY_VAULT_NAME:-none}' does not have '$SECRET_NAME')"
fi
